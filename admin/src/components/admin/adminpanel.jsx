...
import {
  ...
  Button, Container, TextField, Box
} from "@mui/material";
  ...
  return (
    <Container className="admin-container">
      ...
      <Box className="search-container">
        <TextField label="Search by Name" fullWidth margin="normal"
          onChange={(e) => setSearch({ ...search, name: e.target.value })} />
        <TextField label="Search by Date" type="date" fullWidth margin="normal"
        ...
        <TableBody>
          {attendance.map((entry) => (
            <TableRow key={entry._id}>
              ...
              <TableCell>
                {entry.photo ? (
                  <img src={`data:image/jpeg;base64,${entry.photo}`} alt="User"
                    width="50" height="50" />
                ) : "No Image"}
              </TableCell>
              <TableCell>{entry.status}</TableCell>
              <TableCell>
                <Button onClick={() => handleStatusChange(entry._id, "On Time")}>
                  On Time
                </Button>
                <Button onClick={() => handleStatusChange(entry._id, "Late")}>
                  Late
                </Button>
              </TableCell>
            </TableRow>
